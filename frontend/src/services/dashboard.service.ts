...
export const dashboardService = {
  ...
  async getRevenueSummary(period: 'day' | 'week' | 'month' = 'day', startDate?: string, endDate?: string) {
    ...
    return {
      ...
      totalRevenue: normalizeNumber(response.totalRevenue),
      totalOrders: response.totalOrders,
      averageOrderValue: normalizeNumber(response.averageOrderValue),
      dailyBreakdown: response.dailyBreakdown.map((day: any) => ({
        date: day.date,
        revenue: normalizeNumber(day.revenue),
        orders: day.orders,
      })),
      paymentBreakdown: Object.entries(response.paymentBreakdown || {}).reduce<Record<string, { revenue: number; orders: number }>>(
        (acc, [method, value]: [string, any]) => {
          acc[method.toLowerCase()] = {
