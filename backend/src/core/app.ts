...
import categoryRoutes from '@api/routes/v1/category.routes';
import orderRoutes from '@api/routes/v1/order.routes';
import stockRoutes from '@api/routes/v1/stock.routes';
import dashboardRoutes from '@api/routes/v1/dashboard.routes';
import userRoutes from '@api/routes/v1/user.routes';
import paymentRoutes from '@api/routes/v1/payment.routes';
import recipeRoutes from '@api/routes/v1/recipe.routes';
import uploadRoutes from '@api/routes/v1/upload.routes';
import reportingRoutes from '@api/routes/v1/reporting.routes';
import customerRoutes from '@api/routes/v1/customer.routes';
...
const envOrigins = env.FRONTEND_URL ? env.FRONTEND_URL.split(',').map(url => url.trim()).filter(Boolean) : [];
const allowedOrigins = [...defaultOrigins, ...envOrigins.filter(url => !defaultOrigins.includes(url))];
...
app.use(
  cors({
    origin: (origin, callback) => {
      ...
      if (env.NODE_ENV !== 'production' && origin === 'http://localhost:3000') {
        return callback(null, true);
      }
