...
export class PaymentController {
  ...
  async createPayment(req: Request, res: Response) {
    try {
      ...
      if (error instanceof z.ZodError) {
        ...
        logger.error('Payment creation error', { error: error.message, stack: error.stack, orderId: req.body.orderId, paymentMethod: req.body.paymentMethod });
  ...
  async handleCallback(req: Request, res: Response) {
    try {
      ...
      logger.error('Payment callback error', { error: error.message, stack: error.stack, query: req.query });
      const frontendUrl = process.env.FRONTEND_URL || 'http://localhost:3000';
