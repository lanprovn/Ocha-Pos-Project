...
interface PrintItemTicketButtonProps {
  order: OrderTracking;
  item: CartItem;
}

const PrintItemTicketButton: React.FC<PrintItemTicketButtonProps> = ({ order, item }) => {
  const handlePrint = () => {
  ...
  return (
    <button
      onClick={handlePrint}
      className="flex items-center space-x-1 px-2 py-1 bg-green-600 hover:bg-green-700 text-white rounded-md transition-colors text-xs"
      title="In thông tin món"
    >
      <PrinterIcon className="w-3 h-3" />
      <span className="hidden sm:inline">In thông tin món</span>
    </button>
  );
};

export default PrintItemTicketButton;
